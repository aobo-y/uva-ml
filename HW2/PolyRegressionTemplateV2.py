...
def train_test_split(x, y, train_proportion):
    ...
    # shuffled = np.random.shuffle(zip(x, y))
    # x_train, y_train = zip(shuffled[:train_size])
    # x_test, y_test = zip(shuffled[train_size:])
    # x_train, y_train, x_test, y_test = [
    #     np.array(_) for _ in (x_train, y_train, x_test, y_test)
    # ]
...
def get_loss_per_poly_order(x, y, degrees):
    ...
    for d in degrees:
        xp_train = increase_poly_order(x_train, d)
        theta = normal_equation(xp_train, y_train)
        training_losses.append(get_loss(y_train, predict(xp_train, theta)))

        xp_val = increase_poly_order(x_val, d)
        validation_losses.append(get_loss(y_val, predict(xp_val, theta)))
...
if __name__ == "__main__":
    ...
    print('test loss:', get_loss(y_test, predict(x_testp, best_theta)))
